...
export const useCabinetStore = defineStore('cabinet', {
  ...
  actions: {
    async fetchCabinetsWithRecords(productionId, startDate, endDate, allDays) {
      try {
        const response = await axios.get(`http://178.72.185.4/api/cabinet-type/production/${productionId}`, {
          params: {
            startDate,
            endDate
          }
    ...
    async createCabinet(name) {
      try {
        const response = await axios.post(`http://178.72.185.4/api/cabinet-type`, {
          name
        });
    ...
    async searchCabinetsByName(name) {
      try {
        const response = await axios.get(`http://178.72.185.4/api/cabinet-type/search`, {
          params: {
            name
          }
        });
    ...
    async patchCabinetRecord(cabinetTypeId, record) {
      try {
        await axios.patch(`http://178.72.185.4/api/production-record/${cabinetTypeId}`, record);
      } catch (error) {
    ...
    async createCabinetRecord(cabinetTypeId, productionId, record) {
      try {
        ...
        const response = await axios.post(`http://178.72.185.4/api/production-record`, body);
        return response.data;
      } catch (error) {
        console.error(`Ошибка при обновлении записи: ${error.message}`);
      }
    ...
    async linkToProduction(cabinetId, productionId) {
      try {
        ...
        await axios.post(`http://178.72.185.4/api/cabinet-type/link-to-production`, body);
      } catch (error) {
    ...
    async unlinkFromProduction(cabinetId, productionId) {
      try {
        ...
        await axios.post(`http://178.72.185.4/api/cabinet-type/unlink-from-production`, body);
      } catch (error) {
        console.error(`Ошибка при связи шкафа с проектом: ${error.message}`);
      }
    },
