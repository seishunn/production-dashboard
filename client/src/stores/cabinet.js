...
export const useCabinetStore = defineStore('cabinet', {
  ...
  actions: {
    async fetchCabinetsWithRecords(productionId, startDate, endDate, allDays) {
      try {
        const response = await axios.get(`http://localhost:3000/cabinet-type/production/${productionId}`, {
          params: {
            startDate,
            endDate
          }
    ...
    async createCabinet(name) {
      try {
        const response = await axios.post(`http://localhost:3000/cabinet-type`, {
          name
        });
    ...
    async searchCabinetsByName(name) {
      try {
        const response = await axios.get(`http://localhost:3000/cabinet-type/search`, {
          params: {
            name
          }
        });
    ...
    async patchCabinetRecord(cabinetTypeId, record) {
      try {
        await axios.patch(`http://localhost:3000/production-record/${cabinetTypeId}`, record);
      } catch (error) {
    ...
    async createCabinetRecord(cabinetTypeId, productionId, record) {
      try {
        ...
        const response = await axios.post(`http://localhost:3000/production-record`, body);
        return response.data;
      } catch (error) {
        console.error(`Ошибка при обновлении записи: ${error.message}`);
      }
    ...
    async linkToProduction(cabinetId, productionId) {
      try {
        ...
        await axios.post(`http://localhost:3000/cabinet-type/link-to-production`, body);
      } catch (error) {
    ...
    async unlinkFromProduction(cabinetId, productionId) {
      try {
        ...
        await axios.post(`http://localhost:3000/cabinet-type/unlink-from-production`, body);
      } catch (error) {
        console.error(`Ошибка при связи шкафа с проектом: ${error.message}`);
      }
    },
